...
if (!empty($_POST["ok_acc"])) {
	...
	if (!empty($_POST["v_quota"]) || !empty($_POST["v_aliases"]) || !empty($_POST["v_fwd"])) {
		...
		// Auto-provision Mail Domain if it does not exist
	if (empty($_SESSION["error_msg"])) {
	...
	// Add Mail Account
	if (empty($_SESSION["error_msg"])) {
		// Ensure mail domain exists automatically
		exec(HESTIA_CMD . "v-list-mail-domain " . $user . " " . $v_domain . " json", $check_mail_dom, $check_mail_rv);
		if ($check_mail_rv > 0) {
			exec(HESTIA_CMD . "v-add-mail-domain " . $user . " " . $v_domain . " 'yes' 'yes' 'yes'");
		}
		unset($check_mail_dom);
