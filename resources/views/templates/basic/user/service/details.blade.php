        ...
        <div id="tab-content-dns" class="tab-pane hidden space-y-6">
            <div class="p-6 bg-[#141416] border border-white/10 rounded-2xl space-y-4">
                <div class="flex items-center justify-between">
                    ...
                    <button onclick="alert('Adding record...')" class="px-3 py-1.5 bg-indigo-600 text-white text-xs font-semibold rounded-lg hover:bg-indigo-500">
                        + @lang('Add Record')
                    </button>
                </div>
            ...
            <div class="p-6 bg-[#141416] border border-white/10 rounded-2xl space-y-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-neutral-400">Let's Encrypt Wildcard SSL is active and renewing automatically.</p>
                    </div>
                    <button onclick="alert('Running AutoSSL check...')" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold rounded-lg flex items-center gap-1.5">
                        <i data-lucide="shield-check" class="w-4 h-4"></i>
...
<div id="createMailModal" class="hidden fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-[#141416] border border-white/10 rounded-2xl p-6 w-full max-w-md space-y-4">
        <div class="flex items-center justify-between">
            <h3 class="text-base font-bold text-white">Create Mailbox</h3>
            <button onclick="document.getElementById('createMailModal').classList.add('hidden')" class="text-neutral-400 hover:text-white">&times;</button>
        </div>
        <div class="space-y-3 text-xs">
            <div>
                <label class="block text-neutral-400 mb-1">Username</label>
                <div class="flex items-center">
                    <input type="text" class="w-full bg-[#1C1C1F] border border-white/10 rounded-l-lg p-2.5 text-white" placeholder="hello">
                    <span class="bg-white/5 border border-l-0 border-white/10 px-3 py-2.5 text-neutral-400">@ {{ $service->domain ?? 'domain' }}</span>
                </div>
            </div>
            <div>
                <label class="block text-neutral-400 mb-1">Password</label>
                <div class="flex gap-2">
                    <input type="text" id="genMailPass" class="w-full bg-[#1C1C1F] border border-white/10 rounded-lg p-2.5 text-white font-mono" value="x9$K#mP2!vL8">
                    <button type="button" onclick="document.getElementById('genMailPass').value = Math.random().toString(36).slice(-10) + '!A9';" class="px-3 py-2.5 bg-white/10 text-white font-semibold rounded-lg">Generate</button>
                </div>
            </div>
        </div>
        <div class="pt-2 flex justify-end gap-2">
            <button onclick="document.getElementById('createMailModal').classList.add('hidden')" class="px-4 py-2 bg-white/5 text-neutral-300 text-xs font-semibold rounded-lg">Cancel</button>
            <button onclick="alert('Mailbox created!'); document.getElementById('createMailModal').classList.add('hidden');" class="px-4 py-2 bg-indigo-600 text-white text-xs font-semibold rounded-lg">Create</button>
        </div>
    </div>
</div>
